...
const MESSAGE_ID_PATTERN: &str =
    r"<[A-Za-z0-9!#%&'*+/=?_{}|~.-]{1,120}@[A-Za-z0-9.-]{1,80}[.][A-Za-z]{2,24}>";
...
impl Rule for MessageIdRule {
    ...
    fn validate(&self, candidate: &Candidate<'_>) -> Option<Verdict> {
        if !candidate.has_cue(MESSAGE_ID_CUES, DEFAULT_CUE_WINDOW) {
            return None;
        }
        ...
        // The same membership test the email rule leans on, and for the same reason: it is what
        // separates a real right-hand side from a file name.
